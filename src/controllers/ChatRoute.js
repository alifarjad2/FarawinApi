...
router.route('/chat').post(authenticate, async (req, res, next) => {
    ...
    sendToEita("Send_chat", "#Send_chat" + "\n" + `to: ${username}\nfrom:${user.username}\n\n${convert(textHtml)}`);
...
router.route('/chat').put(authenticate, async (req, res, next) => {
    /* 
    #swagger.tags = ['Chat']
    ...
    sendToEita("Edit_chat", "#Edit_chat" + "\n" + `id: ${findChat.id}\nto: ${findChat.receiver}\nfrom:${user.username}\n\n${convert(textHtml)}`);
...
router.route('/chat').delete(authenticate, async (req, res, next) => {
    /* 
    #swagger.tags = ['Chat']
    ...
    const result = await writeFile(fileName, chatList.filter(c => c !== findChat));
    if (!result) return res.status(500).json({
        code: '500',
        message: 'خطایی در سمت سرور رخ داد است!',
    })

    sendToEita("Delete_chat", "#Delete_chat" + "\n" + `id: ${findChat.id}\nto: ${findChat.receiver}\nfrom:${user.username}`);
...
router.route('/chat').get(authenticate, async (req, res, next) => {
    /*  
    #swagger.tags = ['Chat']
